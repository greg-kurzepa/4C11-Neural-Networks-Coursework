...
#     r"C:\Users\gregk\Documents\MyDocuments\IIB\4C11\cw2\Coursework2_Problem_1\params",
#     "20250315-193509")
# params_folder = os.path.join(
#     r"C:\Users\gregk\Documents\MyDocuments\IIB\4C11\cw2\Coursework2_Problem_1\params",
#     "20250315-203439")

# load parameters for model trained without data (pure PDE)
params_folder = os.path.join(
    r"C:\Users\gregk\Documents\MyDocuments\IIB\4C11\cw2\Coursework2_Problem_1\params",
    "20250315-175808")
stress_net.load_state_dict(torch.load(os.path.join(params_folder, "stress_net_it10k.torchparams"), weights_only=True))
disp_net.load_state_dict(torch.load(os.path.join(params_folder, "disp_net_it10k.torchparams"), weights_only=True))

# # load parameters for model trained with data (PDE+data)
# params_folder = os.path.join(
#     r"C:\Users\gregk\Documents\MyDocuments\IIB\4C11\cw2\Coursework2_Problem_1\params",
#     "20250318-115801")
# stress_net.load_state_dict(torch.load(os.path.join(params_folder, "stress_net_it10k.torchparams"), weights_only=True))
...
# # load parameters for model trained with data AND (later) without data (PDE+data)
# params_folder = os.path.join(
#     r"C:\Users\gregk\Documents\MyDocuments\IIB\4C11\cw2\Coursework2_Problem_1\params",
#     "20250318-130434")
# stress_net.load_state_dict(torch.load(os.path.join(params_folder, "stress_net_it6k.torchparams"), weights_only=True))
# disp_net.load_state_dict(torch.load(os.path.join(params_folder, "disp_net_it6k.torchparams"), weights_only=True))
...
def closure_eval(data_loss_fix=False):
    # scheduler.step()
...
# Calculate the loss
loss = closure_eval().item()
data_loss = closure_eval(data_loss_fix=True).item()
...
plt.figure(2)
plt.clf()
plt.title(f"PDE Loss: {loss:.4E}\nData Loss: {data_loss:.4E}")
# levels = np.linspace(-0.08, 0.56, 33)
plt.tricontourf(triang,sigma[:,0].detach().numpy(), cmap="plasma", norm=matplotlib.colors.SymLogNorm(linthresh=0.16, vmin=-0.08, vmax=0.56), levels=33)
plt.colorbar()
plt.show()
# %%