...
def run_experiment(config, hyperparams):
    ...
    with warnings.catch_warnings(): # ignore warning that TrainingMode is deprecated
        ...
        torch.onnx.export(
            ...
            f"code\\onnx\\model-{config.name}-{start_time}.onnx",        # filename of the ONNX model
    ...
    _train.train(model=model, loss_function=config.loss_function, optimiser=optimiser, train_dl=train_dl, valid_dl=valid_dl, dev=dev, **hyperparams["hyperparams"], show_plot=False, log_wandb=config.log_wandb)
...
import configs._config_q2 as _config
config = _config.CONFIGS["cw1_q2_fcnn"]()
